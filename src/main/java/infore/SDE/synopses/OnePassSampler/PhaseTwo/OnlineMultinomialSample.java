...
/**
 * Final result of OnlineMultinomialSampler.
 *
 * samples:
 *   The final with-replacement multinomial sample.
 *   Duplicates are allowed and must be preserved.
 *
 * orderedReservoir:
 *   The intermediate ordered weighted-without-replacement reservoir.
...
public final class OnlineMultinomialSample<T> implements Serializable {
    ...
    public OnlineMultinomialSample(
            int requestedSampleSize,
            long itemsSeen,
            long positiveItemsSeen,
            double totalWeight,
            List<T> samples,
        ...
        this.orderedReservoir = Collections.unmodifiableList(
                new ArrayList<WeightedReservoirEntry<T>>(orderedReservoir)
        );
    }
    ...
    public List<WeightedReservoirEntry<T>> getOrderedReservoir() {
        return orderedReservoir;
    }

    public boolean hasFullSample() {
        return samples.size() == requestedSampleSize;
    }

    @Override
    public String toString() {
        return "OnlineMultinomialSample{" +
                "requestedSampleSize=" + requestedSampleSize +
                ", itemsSeen=" + itemsSeen +
                ", positiveItemsSeen=" + positiveItemsSeen +
                ", totalWeight=" + totalWeight +
                ", sampleCount=" + samples.size() +
                ", reservoirCount=" + orderedReservoir.size() +
                '}';
    }
}