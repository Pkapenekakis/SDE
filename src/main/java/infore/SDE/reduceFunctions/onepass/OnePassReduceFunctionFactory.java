...
//Chooses the correct OnePass reducer from local result type/request ID.
public final class OnePassReduceFunctionFactory {
    ...
    public static ReduceFunction create(Estimation value) {
        ...
        if ("LOCAL_PHASE1_RESULT".equals(type) || value.getRequestID() == 72) {
            return new OnePassIndexReduceFunction(value.getNoOfP(), 0, value.getParam(),
                    value.getSynopsisID(), value.getRequestID());
        }

        if ("LOCAL_PHASE1_SHARD_READY".equals(type) || value.getRequestID() == 76) {
            return new OnePassWorkerReadyReduceFunction(value.getNoOfP(),
        ...
        if ("LOCAL_PHASE2_ROOT_SUMMARY".equals(type) || value.getRequestID() == 82) {
            return new OnePassSampleReduceFunction(value.getNoOfP(), 0, value.getParam(),
                    value.getSynopsisID(), value.getRequestID());
        }

        /* New coordinator-free sharded Phase-3 strict-union path. */
        if ("LOCAL_PHASE3_ALIAS_SELECTIONS".equals(type) || value.getRequestID() == 87) {
            return new OnePassShardedAliasReduceFunction(value.getNoOfP(), 0, value.getParam(),
                    value.getSynopsisID(), value.getRequestID());
        }

        if ("LOCAL_PHASE3_ALIAS_SELECTIONS_INSTALLED".equals(type) || value.getRequestID() == 90) {
            return new OnePassWorkerReadyReduceFunction(value.getNoOfP(), 0, value.getParam(),
                    value.getSynopsisID(), value.getRequestID());
        }

        /* Legacy compatibility path remains separate. */
        if ("LOCAL_PHASE3_ALIAS_RESULT".equals(type) || value.getRequestID() == 92) {
            return new OnePassAliasReduceFunction(value.getNoOfP(), 0, value.getParam(),
                    value.getSynopsisID(), value.getRequestID());
        }
